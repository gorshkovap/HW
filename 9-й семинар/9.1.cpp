...
void merger(T * arr, const int left, const int middle, const int right, F cmp)
{
	std::vector <T> subsidiary_array(right - left + 1);
	for (auto i = 0U; i < right - left + 1; ++i)
	{
		subsidiary_array[i] = arr[left + i];
	}
	int left_from_left = 0;
	int left_from_right = middle - left + 1;
	auto i = 0U;
...
int main()
{
	...
	int_array[3] = 3;
	int_array[4] = 2;
	call_merge_sort(int_array, 5, [](int a, int b) {return a > b; });
	for (auto i = 0U; i < 5; ++i)
	{
